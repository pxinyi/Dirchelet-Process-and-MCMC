# Given Chicago data
Chicago_given=read.csv(file = "Chicago_given.csv")
colnames(Chicago_given)=c("x1","x2")
...
ggplot(data=data)+
    ...
    ggtitle("rescaled Chicago_given data")
...
library(plotly)
contour_range= list(end = max(df$z), size =40 , start = min(df$z))
fig <- plot_ly(
    type = 'contour',
    x=df$x,
    y=df$y,
