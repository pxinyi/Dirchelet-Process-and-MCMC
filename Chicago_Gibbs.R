...
log_marginal_dataM=log_marginal_data(X,nu,lambda,mu0,Chi)
...
# PLOTS #######################################################################################
pdf(file = "(ITER=300)Chicago---Gibbs.pdf") 
...
iter=100
iter_num=NumClusters_trace[iter-1]
mudf=data.frame(matrix(mu_trace[iter-1,1:(2*iter_num)],ncol = 2,byrow = TRUE))
sigmadf=data.frame(matrix(sigma_trace[iter-1,1:(4*iter_num)],ncol = 4,byrow = TRUE))
...
ggplot(data, aes(x = x1, y = x2))+
    geom_point(aes(shape=as.factor(C_trace[iter,]),colour = as.factor(C_trace[iter,])),size=1.3,alpha=1)+
    scale_shape_manual(values=(1:25))+
    coord_fixed(ratio=1)+
